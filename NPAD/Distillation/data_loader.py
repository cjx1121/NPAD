...
def get_train_loader(opt):    # 预处理数据集
    ...
    train_loader = DataLoader(train_data, batch_size=opt.batch_size, shuffle=True, num_workers=0)  # batch_size默认为64

    return train_loader

def get_test_loader(opt):   # 将数据集分成
...
class DatasetCL(Dataset):
    def __init__(self, opt, full_dataset=None, transform=None):
        self.dataset = self.random_split(full_dataset=full_dataset, ratio=opt.ratio)  # 默认为0.05
...
class DatasetBD(Dataset):
    def __init__(self, opt, full_dataset, inject_portion, transform=None, mode="train", device=torch.device("cuda"), distance=1):
        # target_label默认为5， trig_h默认为3 ,trig_w默认为3 , trigger_type默认为grid_trigger , target_type默认为all2one
    ...
    def addTrigger(self, dataset, target_label, inject_portion, mode, distance, trig_w, trig_h, trigger_type, target_type):
        ...
        perm = np.random.permutation(len(dataset))[0: int(len(dataset) * inject_portion)] # 随机打乱数据集
        # dataset
        dataset_ = list()
        ...
        for i in tqdm(range(len(dataset))):
            ...
            if target_type == 'all2one':
                ...
                if mode == 'train':
                    ...
                    if i in perm:
                        ...
                        dataset_.append((img, data[1]))  # 注射率之外的数据标签保持原样
            ...
            # clean label attack
                ...
                if mode == 'train':
                    ...
                    if i in perm:
                        if data[1] == target_label:  # 如果标签等于目标标记 计数器加一
                            # 选择触发器模式
                            img = self.selectTrigger(img, width, height, distance, trig_w, trig_h, trigger_type)
