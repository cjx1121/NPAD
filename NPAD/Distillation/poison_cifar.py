...
def split_dataset(dataset, val_frac=0.1, perm=None):  # 数据集切片 后0.9为训练集，前0.1为验证集
    """
...
def generate_trigger(trigger_type):   # 生成trigger，以及trigger对应位置掩码全为1
...
def add_trigger_cifar(data_set, trigger_type, poison_rate, poison_target, trigger_alpha=1.0): # 给中毒图片加trigger，
    """
    ...
    print("choices:{}".format(choices), len(choices))
    for idx in choices:
...
def add_predefined_trigger_cifar(data_set, trigger_info, exclude_target=True):  # 给中毒图片加trigger
    """
...
if __name__ == '__main__':
    ...
    print(poison_set.__getitem__(1))
    x, y = poison_set.__getitem__(1)
    plt.imshow(x)
    # plt.show()
    plt.savefig('z.png')
