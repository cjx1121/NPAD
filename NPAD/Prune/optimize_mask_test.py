...
def main():
    ...
    orig_train = CIFAR10(root=args.data_dir, train=True, download=True, transform=transform_train)  # 原始训练数据
    _, clean_val = poison.split_dataset(dataset=orig_train, val_frac=args.val_frac,
                                        perm=np.loadtxt('./data/cifar_shuffle.txt', dtype=int))  # val_frac默认为1%
    ...
    for i, (images, labels) in enumerate(data_loader):
        ...
        if args.anp_eps > 0.0:
            ...
            for _ in range(args.anp_steps):
                ...
                include_noise(model)  # 启用特定类型的批归一化层（NoisyBatchNorm2d和NoisyBatchNorm1d）中的噪声
                output_noise = model(images)
                loss_noise = - criterion(output_noise[4], labels)

                loss_noise.backward()
                sign_grad(model)       # 针对带有'neuron_noise'名称的参数，将梯度值转换为它们的符号
                noise_opt.step()
        ...
        if args.anp_eps > 0.0:
            include_noise(model)
            output_noise = model(images)
            loss_rob = criterion(output_noise[4], labels)
        else:
            loss_rob = 0.0

        exclude_noise(model)
        output_clean = model(images)
        loss_nat = criterion(output_clean[4], labels)
        loss = args.anp_alpha * loss_nat + (1 - args.anp_alpha) * loss_rob

        pred = output_clean[4].data.max(1)[1]
